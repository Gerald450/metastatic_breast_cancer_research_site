...
const colorClasses: Record<'pink' | 'blue', { bg: string; text: string; border: string }> = {
  pink: {
      ...
      {topics.map((topic) => {
        ...
        return (
          <Link
            key={topic.href}
            href={topic.href}
            className={`card card-hover group flex items-start gap-4 rounded-xl border bg-white p-5 dark:bg-gray-800/50 ${c.border} transition-all duration-200 hover:-translate-y-0.5 hover:border-pink-300/60 dark:hover:border-pink-700/40`}
          >
            ...
            <svg
              className="h-5 w-5 shrink-0 text-gray-400 transition group-hover:translate-x-1 group-hover:text-pink-600 dark:group-hover:text-pink-400"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden
